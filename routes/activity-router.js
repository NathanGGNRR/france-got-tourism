...
/**
 * -- Doing --
 * Display activity view to the client with ID activity into URL.
...
router.get('/:activityId', function (req, res, next) {
  ...
  fetch('http://vps.cours-diiage.com:8080', { //HTTP Request, ask API of VPS with the query in the body.
    ...
    .then(data =>
      res.render('layout/activity', { data: data.data.poi.results[0] })); //Wait the JSON format and display activity view with all information of the poi
});
